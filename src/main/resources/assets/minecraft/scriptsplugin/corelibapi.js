...
Core.registerCommand = function(c, stale) {
    ...
    var command = new(Java.extend(Command))(aliases[0], Java.to(aliases, "java.lang.String[]")) {
        execute: function(args) {
            try {
                args = Java.from(args);
                var formattedArgs = args.toLowerCase().slice(1),
                    scope, subcommands, parametersMap, scopePos = 1;
...
Chat = chat = {
    print: print = function() {
        ClientUtils.displayChatMessage(getArguments(arguments).join(" "))
    }
};
...
((_method = getMethods(clazz).find(function(m) m.getName() == name && (!argumentArr || (m.getParameterCount() == argumentArr.length && !Java.from(m.getParameterTypes()).some(function(clazz, i) {
...
function canStep(stepHeight, predictDistance) {
    if (mc.thePlayer.isCollidedHorizontally || predictDistance) {
        var yaw = MovementUtils.getDirection(),
            bb = mc.thePlayer.getEntityBoundingBox(),
            possiblePlaces = [],
            otherBB, predictDistance = predictDistance || 0.01;
        for (var i = 0;
            (i += 0.125) <= stepHeight;) {
...
function EditableEnum(target) {
    ...
    var accessor = getMethod(java.lang.reflect.Constructor, "acquireConstructorAccessor").invoke(getConstructor(target, 0)),
        a;
...
function Reflector(object) object && object instanceof java.lang.Object ?
    new JSAdapter() {
        ...
        __call__: function(name) {
            switch (name) {
                case "toString":
                    return object + "" //.toString() doesn't work for java beans
                case "valueOf":
                    return object;
                default:
...
Object.defineProperty(Array.prototype, "find", {
    value: function(func, returnIndex) {
        for (var i in this)
            if (func(this[i], i, this)) return returnIndex ? +i : this[i];
